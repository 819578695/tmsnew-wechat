  ...
  methods:{
    getLoadOrder (val){
      ...
      this.utils.request.queryOrderInfoByNo({'order_no':'O201810211125030010'},function(data){
        ...
        this_.utils.request.getQueryList({'t':'order','id':data.id},function(dataInfo){
            this_.details.paremes=dataInfo[0];
            this_.utils.request.queryPage({'t':'orderProduct','order_id':p,start:1,limit:1000},function(dataProduct){
