  ...
  methods:{
    getLoadOrder (val){
      ...
      this.utils.request.queryOrderInfoByNo({'order_no':'154415163373400000003'},function(data){
          ...
          this_.utils.request.queryInfo({'t':'order','id':this.id},function(dataInfo){
            this_.data=dataInfo;
            this_.data.products=[];
            this_.utils.request.queryPage({'t':'orderProduct','order_id':this.id,start:1,limit:1000},function(dataProduct){
              this_data.products=dataProduct
            });
        });
      });
    },
    toggleDetails(p){
      this.$router.push({name: 'orderDetails',query:{data:this_data}});
    }
  }
}
</script>
