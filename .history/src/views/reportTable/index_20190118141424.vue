      ...
      <van-row style="padding-top:25px;" v-show="type==1">
        ...
        >
          ...
          <div style="width:100%;height: 55px;">
            ...
            >{{item.license}}</span>
            <div :class="item.bname=='配载完成'||item.bname=='已中转'?'isNoClick':'isOkClick'" :status='item.upstatus' @click="item.bname=='配载完成'||item.bname=='已中转'?update():updateStatus(item.upstatus)">{{item.bname}}</div>
          </div>
...
export default {
  ...
  methods: {
    ...
    updateStatus(val){
      this.tips={title: '提示', content: "提交成功", show:true};
    }
  }
};
</script>
