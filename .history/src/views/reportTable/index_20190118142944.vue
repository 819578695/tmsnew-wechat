      ...
      <van-row style="padding-top:25px;" v-show="type==1">
        ...
        >
          ...
          <div style="width:100%;height: 55px;">
            ...
            >{{item.license}}</span>
            <div :class="item.bname=='配载完成'||item.bname=='已中转'?'isNoClick':'isOkClick'" :status='item.upstatus' @click="item.bname=='配载完成'||item.bname=='已中转'?updateStatus(1):updateStatus(item.upstatus)">{{item.bname}}</div>
          </div>
