<template>
  <div>
    <div @click="$router.push({name:'orderInfo',query:{id:1}})"><img style='width:40px;height:40px;float: right;transform: translate(-15px,-44px);' src="../../assets/images/ewm.png"/></div>
    ...
    window.getToAddress = (address='') => {
      if(null!=address){
        //   var last=address.substr(index,address.length);
          ...
          var barcode=address;
          this.utils.request.queryOrderByDriver({'order_no':barcode,'driver_id':driver_id},function(data){
