      ...
      <van-row id="rows">
        <div style="height:140px;width:95%;margin-bottom:15px;box-shadow:rgba(0, 0, 0, 0.3) 0px 4px 6px 2px;"  v-for="(item, index) in orderList" :key="index">
          <div style="width:100%;height: 15%;margin-bottom: 30px;padding-top: 15px;">
            <div style="width: 55%;float: left;text-align: left;padding-left: 30px;">
          ...
          <div style="font-size: 14px;width:90%;height: 30px;">
            <button style="background-color:rgb(240, 238, 242);float: right;border:none;height: 30px;width: 80px;font-size: 13px;box-shadow:2px 4px 6px rgba(0, 0, 0, 0.3);border-radius: 6px;" @click="$router.push({name:'orderInfo',query:{id:item.orderid}})">查看详情</button>
          </div>
        </div>
      </van-row>
      </div>
...
export default {
  ...
  mounted(){
    ...
    this.$store.commit('changeParame', {headerBack:true,hederContent:'运单列表',bottom:'1',bottomShow:false})
  },
  methods:{
    getOrderList(param){
      var this_ = this;
