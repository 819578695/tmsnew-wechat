        ...
        >
          ...
          <div style="width:100%;height: 55px;">
            <span
              style="float: left;width: 33%;font-size: 19px;color:rgb(30, 131, 255)"
            >{{item.license}}</span>
            <div :class="item.bname=='配载完成'||item.bname=='已中转'?'isNoClick':'isOkClick'" :status='item.upstatus' @click="item.bname=='配载完成'||item.bname=='已中转'?updateStatus(1):updateStatus(item.upstatus)">{{item.id,item.bname}}</div>
          </div>
          <div style="width:100%;height: 30px;">
            <span style="font-size: 14px;float: left;width: 65%;">开始时间：{{item.create_time}}</span>
          </div>
