<template>
  <div>
     <tips
      ...
      @close="closeTips"
    ></tips>
    <div><img style='width:36px;height:36px;float: right;transform: translate(-15px,-42px);' src="../../assets/images/ewm.png"/></div>
        ...
        //   var last=address.substr(index,address.length);
          ...
          this.utils.request.queryOrderByDriver({'order_no':barcode,'driver_id':localStorage.getItem('id')},function(data){
            if(null!=data && data=='0001'){
              ...
              this_.$router.push({name:'orderInfo',query:{id:data}})
            }
         });
      }
    };
