...
export default {
  ...
  mounted() {
    ...
    this.utils.request.quertUserInfoByToken({token:this.token},function(data){
        ...
        this_.utils.request.queryShippingByDriver({'driver_id':localStorage.getItem('id')},function(data2){
          if(null!=data2){
            this_.deliverCount=data2.deliverCount;
            this_.queryPickupCount=data2.queryPickupCount;
            this_.queryVehicleCount=data2.queryVehicleCount;
          }
        });
    });
  },
