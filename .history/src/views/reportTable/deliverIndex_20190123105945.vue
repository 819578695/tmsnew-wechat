<template>
  ...
  <div slot="orderList">
    ...
    >
      ...
      <van-row style="padding-top:25px;margin-bottom: 50px;overflow-y: auto;" v-show="type==1">
        <div
        ...
        >
          ...
          <div style="width:100%;height: 55px;">
            <p style="float: left;width: 50%;">{{item.deliver_no}}</p>
          </div>
          <div style="width:100%;height: 55px;">
            <span
              style="float: left;width: 33%;font-size: 19px;color:rgb(30, 131, 255)"
            >{{item.license}}</span>
            <div :class="item.bname=='配送完成'?'isNoClick':'isOkClick'" :status='item.upstatus' @click="item.bname=='配送完成'?updateStatus(0,1):updateStatus(item.id,item.upstatus)">{{item.bname}}</div>
          </div>
          <div style="width:100%;height: 30px;">
            <span style="font-size: 14px;float: left;width: 65%;">配送时间：{{item.create_time}}</span>
          </div>
          <div style="font-size: 14px;width:35%;height: 30px;float: right;">
            <button
              ...
              @click="$router.push({name:'orderList',query:{id:item.id,type:3}})"
            >运单({{item.countorder}})</button>
          </div>
        </div>
      </van-row>
      <van-row style="padding-top:25px;margin-bottom: 50px;overflow-y: auto;" v-show="type==2">
        <div
          style="height:245px;width:95%;margin-bottom:15px;box-shadow:rgba(0, 0, 0, 0.3) 0px 4px 6px 2px;"
          v-for="(item, index) in orderList" :key="index"
        >
      ...
      <van-row style="padding-top:25px;margin-bottom: 50px;overflow-y: auto;" v-show="type==3">
        <div
          style="height:245px;width:95%;margin-bottom:15px;box-shadow:rgba(0, 0, 0, 0.3) 0px 4px 6px 2px;"
          v-for="(item, index) in orderList" :key="index"
        >
...
export default {
  ...
  mounted() {
    this.getShippingLists({ sign: "qazwsxedc", type: 3, bizId: localStorage.getItem('id') });
  ...
  methods: {
    ...
    updateStatus(id,val){
      var this_ = this;
      if(id!=0){
        var param={sign:'qazwsxedc',type:3,bizId:id,status:val};
    ...
    refresh() {
      ...
      return new Promise((resolve, reject) => {
        setTimeout(() => {
          if(this_.type==1){
              this_.getShippingLists({ sign: "qazwsxedc", type: 3, bizId: localStorage.getItem('id') });
