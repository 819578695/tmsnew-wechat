    ...
    <van-row class="bg" type="flex" justify="center" style="border-bottom:1px solid #ccc;">
       <div>
        <van-col span="24" align="center">
          <input type="text" v-model="searchValue" placeholder="点击搜索公司名称" @change="getLoadAlls(this.searchValue)">
        </van-col>
      </div>
    </van-row>
...
export default {
  ...
  mounted(){
    var this_=this;
    this.$store.commit('changeParame', {headerBack:true,hederContent:'订单查询',bottom:'1',bottomShow:false})
  },
  methods:{
    toggleDatetime (p) {
    ...
    getLoadOrder (val){
      this.utils.request.queryOrderInfoByNo({'':val},function(data){
