...
export default {
  ...
  created(){
    ...
    window.getToAddress = (address='') => {
      if(null!=address){
          ...
          var last=address.substr(index-20,index+1);
          var barcode=last.substr(0,last.indexOf('bar'));
          alert(index);
          alert(last);
          alert(barcode);
          this.utils.request.queryProductByCode({'product_no':last},function(data){
  ...
  methods: {
    getLoadOrder(val,type) {
      var this_ = this; 
        this.utils.request.queryOrderNumAndCodeNumById({ id: val,type:type,siteId:localStorage.getItem('siteId')},function(data) {
