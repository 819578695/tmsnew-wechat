            ...
            <div class="btn-1 psd" style="overflow-y: auto;max-height: 300px;">
              ...
              <div style="margin-top: 15px;border: 1.5px solid #00000024;" name="orders" v-for="(val,index) in array" :key="index">
                ...
                <div style="margin-top: 12px;padding-left: 10px;padding-bottom: 12px;">
                  <span>重量：</span><input type="number" name="weight" :placeholder="`默认重量 ${orderDatas.weight}`" style="height: 28px;width: 193px;"/>
                </div>
              </div> 
            </div>
    ...
    confirm () {
      ...
      if(numbers.length>0 && weights.length>0){
          for(var i=0;i<numbers.length;i++){
            numTotal+=parseInt(numbers[i].value);
          }
          for(var i=0;i<weights.length;i++){
            weightTotal+=parseInt(weights[i].value);
          }
          if(''==numTotal?0:numTotal>this.splitRequest.goods_num){
              this.tips={title: '提示', content: "输入件数，不能超过总件数", show:true};
              return false;
          }
          if(''==weightTotal?0:weightTotal>this.splitRequest.weight){
              this.tips={title: '提示', content: "输入重量，不能超过总重量", show:true};
              return false;
          }
          if(''==numTotal?0:numTotal!=this.splitRequest.goods_num){
          ...
          if(''==weightTotal?0:weightTotal!=this.splitRequest.goods_num){
              this.tips={title: '提示', content: "拆单重量必须等于总重量", show:true};
              return false;
          }
          debugger
          var orders=document.getElementsByName('orders');
          return false
          this.$emit("checkChaiOrder",this.num,this.dataRequest.product_no);
      }else{
        this.tips={title: '提示', content: "拆单数量和重量不能为空!", show:true};
        return false;
      }
    },
     closeTips(data) {
