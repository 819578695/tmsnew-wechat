<template>
  <div class="page1">
    ...
    <van-row id="bottom">
        <van-col style="width:100%" style="border-left: 0.0625rem solid #ccc;"><div @click="getBeCode(2)">出库扫码</div></van-col>
      </van-row>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    getLoadOrder(val,type) {
      var this_ = this; 
      if(type=='type'){
        this.utils.request.queryOrderNumAndCodeNumById({ id: val,type:type},function(data) {
          this_.orderData=data;
        });
      }else if(type=='type2'){
        this.utils.request.queryOrderNumAndCodeNumById({ id: val,type2:type},function(data) {
          this_.orderData=data;
        });
      }else if(type=='type3'){
        this.utils.request.queryOrderNumAndCodeNumById({ id: val,type3:type},function(data) {
          this_.orderData=data;
        });
      }
    },
