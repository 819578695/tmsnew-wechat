...
export default {
  ...
  methods: {
    ...
    getBeCode (type){
      ...
      this.utils.request.queryProductByCode({'product_no':'2154211010001-01'},function(data){
    ...
    check(num,product_no){
      this.operationBegin('2154211010001',this.type,num);
    ...
    splitOrder(order_no,goods_num,weight,barcode_num,vehicle_id){
      //如果已经扫码完成，将不能拆分订单
      if(goods_num==barcode_num){
          this.tips={ title: '提示', content: '扫码已完成，不能拆分订单', show: true};
          return false;
      }
      this.splitRequest={order_no:order_no,goods_num:goods_num,weight:weight,vehicle_id};
      this.checkAlertShow=true;
    },
    //拆单
    checkChaiOrder(data,order_no){
      var this_=this;
      this.utils.request.orderSplit({orders:JSON.stringify(data),order_no:order_no},function(data){
        this_.getLoadOrder(this_.order_id);
        this_.tips={ title: '提示', content: data, show: true};
      });
    }
