    ...
    ></tips>
         <div class="con">
            ...
            <div class="btn-1 psd" style="overflow-y: auto;max-height: 300px;">
              ...
              <div style="margin-top: 15px;border: 1.5px solid #00000024;" v-for="(val,index) in num" :key="index">
                <div style="padding-bottom: 10px;font-size:17px;color:red"><span>订单{{index+1}}：</span></div>
                <div>
                  <span>件数：</span><input type="number" :placeholder="`默认件数 ${val.number}`" style="height: 28px;width: 193px;"/>
                </div>
                <div style="margin-top: 15px;">
                  <span>重量</span><input type="number" :placeholder="`默认重量 ${val.weight}`" style="height: 28px;width: 193px;"/>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    changesOrder(){
      this.orderDatas.number=this.splitRequest.goods_num/this.num;
      this.orderDatas.weight=this.splitRequest.weight/this.num;
    }
  }
};
</script>
