...
export default {
  ...
  created(){
    var this_=this;
    window.nativeTransactionCallback =function(obj,code){
        var obj=JSON.parse(obj);
        if(obj.type==1){
          //出库
          this_.operationBegin(code,1);
        }else{
          //入库
          this_.operationBegin(code,2);
        }
    };
  },
  mounted() {
  ...
  methods: {
    ...
    check(num,product_no){
      this.operationBegin('2154211010001-01',this.type,num);
