~<template>
  <div id="app">
    ...
    <div v-show="$store.state.isLogin && $store.state.default.bottomShow">
      <div class="support"></div>
      <van-row id="bottom">
        <van-col span="8" :class="{active:$store.state.default.bottom == 0}"><div @click="jump('order')">当前定位</div></van-col>
        <van-col span="8" :class="{active:$store.state.default.bottom == 1}"><div @click="jump('reporting')" class="bor">运输管理</div></van-col>
        <van-col span="8" :class="{active:$store.state.default.bottom == 2}"><div @click="jump('user')">我的</div></van-col>
      </van-row>
    </div>
  </div>
</template>
