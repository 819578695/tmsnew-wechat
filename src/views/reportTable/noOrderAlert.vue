<template>
  <div class="tips">
    ...
    <tips v-if="tips2.show" :title="tips2.title" :content="tips2.content" @close="closeTips2"></tips>
      ...
      <div class="con">
        ...
        <div class="btn-1 psd" style="overflow-y: auto;max-height: 80%;min-height:60%">
          <div style="margin-top: 15px;">
            ...
            <input
              type="number"
              :placeholder="`总件数 ${splitRequest.totalCount}`"
              style="height: 28px;width: 180px;" v-model="num"
            >
          </div>
          <div style="margin-top: 15px;">
            <span>拒收类型：</span>
        ...
        <div class="foot">
          ...
          <span class="affirm" @click="submit">确认</span>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      tips2: { title: "提示", content: "提示内容", show: false },
      num: '',
      arrayType: [],
      imgBase64: [], //存储img base64的值将值传给后端处理
      imgUrl: [],
      orderid:this.utils.getRequestParam('id'),
      type:''
    };
  },
  mounted(){
  ...
  methods: {
    ...
    getImgBase() {
      ...
      var options = {  // 设置axios的参数
        //  url: this.utils+'/orderBack/imgUpload',
         url:'http://tms-api.zgtgroup.com/orderBack/imgUpload',
         data: formData,
         method: 'post',
         headers: { 
    ...
    submit(){
      if(this.num<=0){
          this.tips={title: '提示',content: "请输入拒收件数！", show:true};
          return false;
      }
      if(this.type<=0){
          this.tips={title: '提示',content: "请选择异常类型！", show:true};
          return false;
      }
      var this_=this;
       this.utils.request.signOrderExce({t:'execOrder',order_id:this.orderid,images:this.imgUrl.join(","),reason:this.type,remark:'拒收数量：'+this.num,totalCount:this.splitRequest.totalCount,num:this.num},function(data) {
         this_.tips2={title: '提示',content: "操作成功", show:true};
      });
    }
  }
};
